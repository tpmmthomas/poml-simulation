...
The commands below were verified on 2026-09-22 with Python 3.11 on Linux, the
...
| Check | Result / scope |
| --- | --- |
| Automated tests | 138 passed, including protocol rejection, arithmetic, cryptographic plumbing, fitting, CLI, CPU PoW, local GPT-2, and DDPM scheduler checks |
...
experiment, the 100-example-per-task LLM Compatibility experiments, the full
Wasted Work Analysis grid, and the 50-block fresh-proof and hashing campaign
were not rerun during this verification pass. Their commands expose the paper's
defaults; reduced runs validate execution and metric semantics only.
