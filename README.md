...
| **Proof chain binding** $\mathsf{bind}_i = H(\pi_{i-1}^{(1)})$ for $i \geq 2$ linking proofs sequentially | **Not implemented** | The simulation evaluates the lottery on the full tuple $\Pi_i$ which implicitly orders proofs, but individual proof-to-proof chaining via $\mathsf{bind}_i$ is not enforced. |
| **Output encryption** $\mathsf{ct}_i = \mathsf{Enc}(\mathsf{pk}_u, y_i \| \mathsf{bind}_i \| \mathsf{taskID})$ | **Plaintext outputs** | Encryption is orthogonal to the consensus mechanism. Outputs are stored as plaintext floats. |
| **ZKP statement** $\mathcal{L}_{\text{PoML}}$ proving inference correctness, commitment opening, model commitment opening, and encryption correctness | **EZKL proof of inference only** | EZKL proves that the ONNX model was correctly evaluated on the given public input. Commitment openings and encryption are not part of the EZKL circuit. |
