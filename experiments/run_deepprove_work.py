...
def validation_pairs() -> list[tuple[int, int]]:
    ...
    return boundaries + off_boundaries + different_splits + fresh_tokens
...
def plot_counts(rows: list[dict], output: Path) -> None:
    ...
    for index, (values, title, label) in enumerate(metrics, 1):
        ...
        ax.set(
            xlabel="Input tokens N", ylabel="Output tokens K", zlabel=label, title=title
        )
...
def analyze(
    ledger: Path, output: Path, provenance: dict, comparison: Path | None = None
) -> dict:
    ...
    schedule = (
        ...
        if comparison
        else compile_schedule(rows, provenance)
    )
    ...
    for row in rows:
        ...
        if (
            row["meta"].get("schema") != SCHEDULE_VERSION
            or row["meta"]["setup_max"] != 64
        ):
        ...
        if error > MAX_COMPONENT_ERROR:
            raise ValueError(
                f"Proof component error {error:.2%} exceeds tolerance at {(n, k)}"
            )
        comparisons.append(
            {
                "n": n,
    ...
    report = {
        ...
        "trials": len(rows),
        "all_exact": all(
            r["inference_equal"] and r["proof_equal"] for r in comparisons
        ),
...
def run(args) -> None:
    ...
    if args.pairs:
        ...
        if max(n + k for n, k in pairs) != 64:
            raise ValueError(
                "Custom pairs must include total length 64 to preserve setup capacity"
            )
    if args.plan_only:
        print(
            json.dumps(
    ...
    if args.analyze:
        ...
        print(
            json.dumps(
                analyze(args.analyze, output, saved, args.compare_schedule), indent=2
            )
        )
        return
    if args.calculate:
    ...
    if any(r["meta"]["backend"] != expected_backend for r in rows):
        raise ValueError(
            "Binary backend does not match --cuda; rebuild without --skip-build"
        )
        ...
        json.dumps(
            {
                ...
                "all_exact": report["all_exact"],
                "max_proof_component_relative_error": report[
                    "max_proof_component_relative_error"
                ],
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "experiments/results/deepprove_work/audited",
    )
    ...
    parser.add_argument(
        "--pairs", help="optional N:K,N:K,... list; must include N+K=64"
    )
    parser.add_argument(
        "--compare-schedule",
        type=Path,
