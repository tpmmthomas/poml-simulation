"""Targeted tests for RSA encryption with VRF-derived randomness.

These are focused on the *determinism* property: the lottery hash over
ciphertexts must be uniquely determined by (pk, r_enc, y, taskID). The
r_enc value is the VRF-derived encryption randomness embedded in the
plaintext. Other encryption behaviour is exercised by tests/test_encryption.py.
"""
...
class TestDeterminism:
    def test_repeated_encryption_identical(self):
        ...
        values = _values(64)
        enc_randomness = b"\x01" * 32
        tid = 42
        ciphertexts = {encrypt_output(pk, values, enc_randomness, tid) for _ in range(5)}
        assert len(ciphertexts) == 1

    def test_different_enc_randomness_differs(self):
        pk, _ = generate_encryption_keypair()
...
class TestRoundtripSizes:
    ...
    def test_roundtrip(self, n):
        ...
        tid = n * 7
        ct = encrypt_output(pk, values, enc_randomness, tid)
        assert len(ct) % 256 == 0
        dec_vals, dec_enc_randomness, dec_tid = decrypt_output(sk, ct, n)
        assert dec_tid == tid
